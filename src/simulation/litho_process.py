"""Complete lithography simulation pipeline."""
import numpy as np
import yaml
from pathlib import Path
from .optics import compute_aerial_image_from_mask
...
class LithoSim:
    ...
    def compute_aerial_image(self, mask=None):
        if mask is None:
            mask = self.create_mask()
        aerial = compute_aerial_image_from_mask(
    ...
    def develop_resist(self, aerial_image=None):
        ...
        resist = (aerial_image * self.cfg['dose_mj'] / 20.0 > threshold).astype(np.uint8)
        return resist
